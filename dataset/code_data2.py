...
J_x=10**np.linspace(-4,0,30)
J_d=10**np.linspace(-2,-0.1,28)
num=(J_x.shape[0]*J_d.shape[0])
A=sys.argv[4]
B=sys.argv[5]
C=sys.argv[6]
F0=np.zeros((num,4));Epsilon=np.zeros((num,2))
Epsilon[:,:]=[A,B];F0[:,:]=[0.0001,0,0.1,C];
...
name1='./%s/%s_Para.txt'%(sys.argv[3],sys.argv[3])
L=[Epsilon[0,:],F0[0,3]];
np.savetxt(name1,L,fmt="%s")
if np.int(sys.argv[1])==0:
   np.savez(name,F0=F0,Epsilon=Epsilon)
   #sio.savemat(name1,{'F0':F0,'Epsilon':Epsilon})
time=4000;dt=0.01;num=3990;cut=range(3000,4000,10);
#limit=850
####
MINES=np.array([1])
...
if CODES[NUM]<sur:
   ...
   for i in range(inter): 
       ...
       P,D,I=classify(history,velocity_mean,num);I1=I
       history=history[cut]
       np.savez(name,history=history,velocity_mean=velocity_mean)
       name='./%s/%s_%i_I'%(sys.argv[3],sys.argv[3],CODES[NUM]*inter+i)
       ruler_std=np.ones((100,1));
       for j in range(100):
           P,D,I=classify(history,velocity_mean,j)
           ruler_std[j,:]=I[0,0]
       k=ruler_std;
       np.savez(name,I=I1,k=k)
       tt2=tm.time();
